...
// --- Simple lead notification helper ---
export async function sendLeadNotification(subject: string, bodyText: string): Promise<boolean> {
  if (!process.env.SENDGRID_API_KEY) {
    console.log('SendGrid API key not configured - lead notification not sent');
    return false;
  }

  try {
    const to = process.env.EMAIL_TO || process.env.SENDGRID_TO;
    const from = process.env.EMAIL_FROM || 'no-reply@alchemyunited.com';
    
    if (!to) {
      console.warn('Email recipient not configured (EMAIL_TO missing). Skipping send.');
      return false;
    }
    ...
    await sgMail.send(msg);
    console.log(`Lead notification sent: ${subject}`);
    return true;
